...
const fetchJson = async (url, revalidate) => {
...
const fetchAllRecords = async (resourceId, revalidate) => {
  let offset = 0
  let total = null
  const records = []
...
const filterByYear = (records, dateField, year) =>
  records.filter((record) => getYearFromValue(record[dateField]) === year)
...
export const getProcurementMetrics = async ({ year, revalidate = 3600 }) => {
  const [awardedData, nonCompetitiveData] = await Promise.all([
    fetchAllRecords(AWARDED_RESOURCE_ID, revalidate),
    fetchAllRecords(NON_COMPETITIVE_RESOURCE_ID, revalidate)
  ])
